...
Bitmap::Bitmap(uint16 * imagefile, bool immediate)
{
	// set data from compressed file with header, 
	// if immediate set ->  make buffer and uncompress
	_initialize();
...
void Bitmap::DrawText(Font * font, Rectangle * rect, const char * text, uint16 color)
{
#ifdef _DEBUGME
	...
	uint16 offset = (_width - font->_char_definition_width);
	...
	while (*text)
	{
		character = (*text++)-font->_first_charactercode;

		uint8 * characterptr =	fontptr + (character * (font->_char_definition_width/8) * font->_char_definition_height)  + 10 ;


		uint8 width;

                if (font->_fixed_width) {
                    
                    width = font->_char_width;
		...
		for (y = 0; y < font->_char_definition_height; y++) 
		{
			for (x = 0; x < (font->_char_definition_width / 8); x++)
			{
				pix = *(characterptr++);
...
void Bitmap::DrawRectangle(Rectangle * rect, uint16 color, uint16 width)
{
	...
	bufptr = _image_buffer + (uint32)(rect->Top * _width + rect->Left);
	...
	bufptr = _image_buffer + (uint32)(rect->Top * _width + rect->Left + rect->Width - 1);
	for (i = 0; i < rect->Height; i++) 
	{
		(*bufptr) = color;
		bufptr += offset;
	...
	bufptr = _image_buffer + (uint32)( ( rect->Top + rect->Height -1 ) * _width + rect->Left);
	for (i = 0; i < rect->Width; i++) 
	{
		(*bufptr) = color;
		bufptr++;
...
void Bitmap::FillRectangle(uint16 x1, uint16 y1, uint16 x2, uint16 y2, uint16 color) {
    uint16 cx;
    uint16 cy;
    uint16 * d;
    uint16 lineoffset;
    ...
    lineoffset = _width - (x2 - x1) - 1;
    
    d = (uint16 *) (_image_buffer) + (((y1 -1)*_width + x1));
...
{
// [TODO] uint32 a = pValue | (pValue << 8)  | (pValue << 16)  | (pValue << 24);
//	swiFastCopy(&a, _alpha_buffer, COPY_MODE_WORD | COPY_MODE_FILL | _pixelcount/4 );
    memset(_alpha_buffer,pValue,_pixelcount / 2);
}
...
void Bitmap::CopyBuffer(Bitmap * pTarget)
{
	...
	// scheinbar nur ein Problem mit Kopien WRAM -> VRAM, WRAM -> WRAM l�uft anstandslos
	// Langsam, frameindex f�r 256x192 = 496
	//	swiFastCopy(this->_image_buffer, pTarget->_image_buffer, COPY_MODE_WORD | (_width*_height*2/4) );
			
	// Bester Kompromiss der auf beiden l�uft ist DMA Halfword Copy, 
	// Word Copy macht in Dualis die gleichen Probleme wie FastCopy (MSW/LSW falschrum)
	// Furchtbar langsam, frameindex f�r 256x192 = 790
