...
WConsole::WConsole(Rectangle * pRect, uint16 pRows, uint16 pCols, Font * pFont) : Window(pRect,"wconsole")
{
	...
	_padding = 4;
	_frame_color = RGB15(0xFF,0x00,0x00) | (1 << (uint16)15);
	_frame_width = 1;
...
{ 
	if (!_valid)
	{
		...
		//	_buffer->DrawText(new Font(FontTypeComicSans10), new Rectangle(2,55,110,40), "Comic Sans MS 10pt  auch sehr sch�n", 1 << 15);
...
void WConsole::PaintChar(uint16 row, uint16 col, char c)
{
	...
	Rectangle * targetRect = new Rectangle(
		_padding + col * (_font->_char_width + _font->_spacing_h),
...
void WConsole::Print(const char * pString)
{
	_valid = false;
	while (*pString)
	{
