...
void Window::_initialize(void)
{
	...
	_frame_color = RGB15(255,0,0) | (1 << 15);
        // _frame_color = ( (255 << 10 ) | (0 << 5 ) | 0 ) | (1 << 15);
...
Window::Window(Rectangle * pRect, const char * ptextid)
{
        _initialize();
...
Window::Window(unsigned int pWidth, unsigned int pHeight, uint16 * pBufferPtr, const char * ptextid)
{
        _initialize();
        
	_position = new Rectangle(0,0,pWidth,pHeight);

	_buffer = new Bitmap(pWidth,pHeight,pBufferPtr);
